...
return (new Config())
    ->withProfile(
        (new Profile('default'))
        ->withSuite(
            (new Suite('api_paying_with_stripe_web_elements_during_checkout', ['javascript' => false]))
            ->withPaths('features')
            ->withContexts(
            ...
            ->withContexts(
                'tests.flux_se.sylius_stripe_plugin.behat.context.api.shop.stripe_web_elements',
            )
            ->withFilter(new TagFilter('@paying_with_stripe_web_elements_during_checkout&&@api')),
        ),
    )
;
