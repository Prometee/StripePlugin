...
return (new Config())
    ->withProfile(
        (new Profile('default'))
        ->withSuite(
            (new Suite('ui_paying_with_stripe_checkout_during_checkout'))
            ->withPaths('features')
            ->withContexts(
            ...
            ->withContexts(
                'tests.flux_se.sylius_stripe_plugin.behat.context.ui.shop.stripe_checkout',
            )
            ->withFilter(new TagFilter('@paying_with_stripe_checkout_during_checkout&&@ui')),
        ),
    )
;
