...
1. Install the [Sylius Stripe App][link-sylius-stripe-app-install] on your Stripe account.
...
Several bundled services gained or changed constructor arguments. **You only need to act if you instantiate one of them
directly in PHP, or through a manual service definition that does not reuse / `parent:` the bundled one.** Applications
relying on the shipped definitions are rewired automatically.

- **`StateMachine\PaymentStateProcessor`** — new `StripeStateAppliedCheckerInterface $stripeStateAppliedChecker`
  inserted **before** `array $supportedFactories` (now the sixth argument).
  Add `@flux_se.sylius_stripe.state_machine.stripe_state_applied_checker`.

- **`OrderPay\Provider\WebElements\CaptureHttpResponseProvider`** — new `AppearanceBuilder $appearanceBuilder`
  appended after `Environment $twig`.
  Add `@FluxSE\SyliusStripePlugin\Appearance\AppearanceBuilder`.

- **`Provider\StripeNotifyPaymentProvider`** — new `RefundEventTokenHashResolverInterface $refundEventTokenHashResolver`
  appended after `EventResolverInterface $eventResolver`.
  Add `@flux_se.sylius_stripe.provider.refund_event_token_hash_resolver`.

- **`Notification\LegacyStripeKeyNotificationProvider`** — the four lookup arguments (`$paymentMethodRepository`,
  `$gatewayFactoryNameProvider`, `$legacyKeyDetector`, `$stripeFactoryNames`) are replaced by a single
  `LegacyStripePaymentMethodsProviderInterface $legacyStripePaymentMethodsProvider` (the shared detection logic now lives
  in that provider, also consumed by the admin Twig extension).
  Replace them with `@flux_se.sylius_stripe.stripe.secret_key.legacy_stripe_payment_methods`.

[link-sylius-stripe-app]: https://marketplace.stripe.com/apps/sylius
[link-sylius-stripe-app-install]: https://marketplace.stripe.com/apps/install/link/com.sylius.stripe
